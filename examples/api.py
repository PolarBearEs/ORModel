...
from ormodel import (
    ...
    shutdown_database,  # Needed if manager is used directly sometimes
)
...
async def db_session_middleware(request: Request, call_next):
    ...
    try:
        ...
        print(
            f"--- [Middleware] Request {request.method} {request.url.path}: Exception caught ({type(e).__name__}), raising. ---"
        )  # Debug
    ...
    if response is None:
        ...
        print(
            f"--- [Middleware] Request {request.method} {request.url.path}: Response is None after try block, returning 500. ---"
        )
...
async def read_single_hero(hero_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Internal server error: Multiple heroes found with same ID",
        )
